...
def create_app(DEV: bool = True, wait_timeout: int = 60, drive_timeout: int = 60):
    ...
    if not DEV:
        ...
        pwm_servo_cam_x_y = ppo.pi()
        pwm_servo_cam_x_y.set_mode(servo_cam_x_y, ppo.OUTPUT)
        pwm_servo_cam_x_y.set_PWM_frequency(servo_cam_x_y, 50)
        pwm_servo_cam_z = ppo.pi()
    ...
    def camera():
        ...
        try:
            ...
            if not DEV:
                ...
                t = threading.Thread(target=os.system('uvccapture -v -m -x1280 -y960 -osnap{}.jpg'.format(count)), args=())
                t.start()
